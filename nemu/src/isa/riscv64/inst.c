...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ SEXT(src2, 32));
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | SEXT(src2, 32));
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 & SEXT(src2, 32));
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(dest) = (long long)src1 < (long long)src2);
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = (long long)src1 < (long long)SEXT(src2,32));
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = src1 < src2);
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < SEXT(src2,32));
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->pc += src1==src2 ? src3 : 0lu);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", beq    , B, s->pc += src1!=src2 ? src3 : 0lu);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->pc += (long long)src1 < (long long)src2 ? src3 : 0lu);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->pc += (long long)src1 >= (long long)src2 ? src3 : 0lu);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->pc += src1 < src2 ? src3 : 0lu);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->pc += src1 >= src2 ? src3 : 0lu);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4lu; s->dnpc = s->pc + SEXT(src1, 64););
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, word_t tmp = s->pc + 4lu; s->dnpc = (SEXT(src1,64) + SEXT(src2, 64))&(~1lu); R(dest)=tmp);
